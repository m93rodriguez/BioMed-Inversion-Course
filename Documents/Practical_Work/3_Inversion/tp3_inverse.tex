...
\begin{equation}\label{eq:DA}
	-\nabla^2 \phi(\r) \phi(\r) + k^2(\r) \phi(\r) = \frac{s(\r)}{D}
\end{equation}
where we assume the diffusion coefficient $D\approx 0.4$ cm everywhere. We will use Extended Boundary conditions, where we extend the domain a distance $2D$ in every direction imposing $\phi(\r) = 0$ for $\r$ in the extended boundary, see Fig.~\ref{fig:domain}.
...
We can discretize this equation in the domain $\Omega$ as $\delta\bm\phi= K\x$ , where $\delta\bm\phi$ is the vector of measurements variations, $\delta\mathbf k^2$ is the vector of parameters variations, and $K$ is the sensitivity matrix. 
