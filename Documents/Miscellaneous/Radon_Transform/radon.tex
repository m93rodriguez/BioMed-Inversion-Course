...
To get all the information along direction $\ori$, we consider all lines that do not intersect. We can do this by choosing a normal vector perpendicular to $\ori$, such that $\r_0 = t\n$ letting $t\in(-\infty, \infty)$.

\begin{equation}
	\log\left(\frac{I(\r_0,\ori)}{I(\r, \ori)}\right) = \int_{-\infty}^{\infty} \mu_a(t\n + s\ori) \dif s
\end{equation}
We can define the Radon transform as
\begin{equation}
	\mathcal R(\n, t) \triangleq \int_{-\infty}^{\infty} \mu_a(t\n + s\ori) \dif s
\end{equation}

If we choose a coordinate system $\r = (x, y)$, then we can parametrize $n$ using some angle $\theta$ with respect to $x$:
\begin{equation}
	\n = \left[\begin{array}{c} \cos\theta \\ \sin\theta \end{array}\right] \quad , \quad \ori = \left[\begin{array}{c} \sin\theta \\ -\cos\theta \end{array}\right]
\end{equation}
Note that the choice of $\n$ is not unique (can be mirrored). The Radon transform is then
\begin{equation}
	\log\left(\frac{I(\r_0,\ori)}{I(\r, \ori)}\right) = \mathcal R(\theta, t) 	\triangleq   \int_{-\infty}^\infty \mu_a(s\sin\theta + t\cos\theta, -s\cos\theta + t\sin\theta) \dif s 
\end{equation}
...
\begin{equation}
	\Pi(\theta, \omega) = \mathcal F\left\{ \mathcal R(\theta,t)\right\} = \int_{-\infty}^\infty \mathcal R(\theta, t) e^{-2\pi i \omega t}  \dif t = M(\omega\n)
\end{equation}
Where the last equality comes from the Fourier slice theorem. Indeed, let $\r = t\n + s\ori$
\begin{align}\begin{split}
	M(\omega\n) =&  \int_\mathbb {R}\int_\mathbb{R} \mu_a(\r) e^{-2\pi i (t\n + s\ori) \cdot (\omega \n)} \dif t \dif s \\
	=& \int_\mathbb {R}\int_\mathbb{R} \mu_a(\r) e^{-2\pi i t\omega} \dif t \dif s \\
	=& \int_\mathbb {R} \left ( \int_\mathbb{R} \mu_a(t\n + s\ori)  \dif s \right ) e^{-2\pi i t\omega}\dif t \\
	=& \int_\mathbb {R} \mathcal R(\n, t) e^{-2\pi i t\omega}\dif t \\
	=& \Pi(\theta, \omega)	
\end{split}\end{align}
...
\end{equation}
consider a change of variables from cartesian to polar coordinates in the frequency plane, using the angle $\theta$ and the `radius' $\omega$, such that $\bm \omega = (\omega\cos\theta, \omega\sin\theta) = \omega\n$:
\begin{align}
	\mu_a(\r) =& \int_0^\pi \int_\mathbb R M(\omega\n) e^{2\pi i \omega \n \cdot \r} |\omega |\dif \omega \dif \theta \\
